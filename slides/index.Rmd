...
subtitle: "Descripción y exploración de datos"
author:
...
- **.bg-yellow[Diapositivas de R]**
<https://dadosdelaplace.github.io/teaching-exploratory/slides>

- **.bg-orange[Entregas de R]**: <https://github.com/dadosdelaplace/teaching-exploratory/tree/main/eval>
...
#### [Bloque I: introducción a R desde cero](#bloque-1)

#### [Bloque II: vectores y matrices](#bloque-2)

#### [Bloque III: conjuntos de datos](#bloque-3)

#### [Bloque IV: comunicando resultados (Rmarkdown)](#bloque-4)

]

.pull-right[

#### [Bloque V: funciones y listas](#bloque-5)

#### [Bloque VI: tibble y tidydata](#bloque-6)

#### [Bloque VII: depurando, ordenando y resumiendo datos (tidyverse)](#bloque-7)

#### [Bloque VIII: análisis de regresión con R](#bloque-8)

#### [Bloque IX: visualizando datos](#bloque-9)
...
Los únicos **.bg-purple_light[requisitos]** serán:
...
### [Variables y vectores](#variables)

### [Ejercicios vectores](#ejercicios2)
...
# Bloque III: conjuntos de datos y estructuras de control

&nbsp;

### [Conjuntos de datos: data.frame y tibble](#data.frame)

### [Ejercicios data.frame y tibble](#ejercicios-tibble)

### [Estructuras condicionales: if-else](#estructuras-condicionales)

### [Bucles: for, while](#bucles)

### [Ejercicios if-else y bucles](#ejercicios-4)
...
* 📝 **Ejercicio 5**: diseña un bucle `for` de 200 iteraciones que, empezando en un valor inicial de 100 (euros), te sume 3€ si el número actual de la iteración es par, y te reste 5€ si es impar (investiga la función `%%`).
...
# BLOQUE VII: depurando, ordenando y resumiendo datos (tidyverse)
...
* 📝 **Ejercicio 5**: selecciona aquellos personajes de `starwars` de la familia Skywalker. Busca la ayuda de la función `str_detect()` del paquete `stringr`
...
* 📝 **Ejercicio 5**: con los datos obtenidos del ejercicio, comprueba cuáles son las modalidades únicas hay en la variable de color de pelo.
...
* 📝 **Ejercicio 1**: crea una nueva columna que nos guarda en la tabla la estatura en metros. 

* 📝 **Ejercicio 2**: crea una nueva columna a la tabla salida del ejercicio anterior que nos recodifique la variable altura: `"bajito"` para aquellos personajes que midan menos de 1.5m, `"mediano"` para los que midan entre 1.5 y 1.8m, `"alto"` para los que miden entre 1.8 y 2.1m, y `"bicho"` para los que midan más de 2.1m.

* 📝 **Ejercicio 3**: usando la tabla salida del ejercicio anterior, elimina registros con datos ausentes en la variable `birth_year` y filtra solo los 15 personajes más jóvenes.

* 📝 **Ejercicio 4**: de la tabla salida del ejercicio anterior, selecciona solo las variables numéricas y de tipo texto. Define una nueva variable llamada `under_18` que nos recategorice la variable `birth_year`: `TRUE` si es menor de edad y `FALSE` en caso contrario
...
starwars_nueva <-
  starwars_nueva %>%
  mutate(height_recat =
           case_when(height_m <= 1.5 ~ "bajito",
                     # solo llega aquí si arriba ha sido FALSE
                     height_m <= 1.8 ~ "mediano",
                     # solo llega aquí si arriba ha sido FALSE
                     height_m <= 2.1 ~ "alto",
...
* 📝 **Ejercicio 5**: de la base de datos original, determina cuáles son las modalidades únicas que toma la variable `species` (elimina antes registros con ausente en dicha variable). 

* 📝 **Ejercicio 6**: sobre la base de datos original, crea una nueva columna llamada `auburn` (cobrizo/caoba) que nos diga `TRUE` si el color de pelo contiene dicha palabra y `FALSE` en caso contrario. Busca `? str_detect()`.

* 📝 **Ejercicio 7**: sobre la base de datos original, filtra solo aquellos personajes de la familia `"Skywalker"` o `"Antilles"`, selecciona solo las columnas de `name`, `mass`, `height` y `specie`, y renombra a castellano. Tras ello crea una nueva columna llamada IMC definida como el cociente entre el peso y el cuadrado de la estatura (en metros).
...
starwars %>%
  ...
  # de name a mass (todas, las 3 que hay) y además species
  select(name:mass, species) %>%
  rename(nombre = name, peso = mass, estatura = height, especie = species) %>% 
...
* 📝 **Ejercicio 5**: obtén la media de todas las variables numéricas de una vez (quita antes ausentes en esa variables) sin importar cuántas sean.
...
* 📝 **Ejercicio 6**: obtén el personaje más viejo por cada sexo.

* 📝 **Ejercicio 7**: obtén las frecuencias (absolutas) desagregadas por `sex` y `gender` y ordenados de mayor a menor aparición (investiga `? count`)

* 📝 **Ejercicio 8**: añade una nueva variable llamada `human_or_not`, que tenga un `TRUE` si la especie es humana, y un `FALSE` en caso contrario. Tras calcula la media de estatura desagregada en los dos estratos: en humanos y en no humanos.

* 📝 **Ejercicio 9**: haciendo uso de la tabla del ejercicio anterior, selecciona el personaje humano más bajito y el personaje no humano más bajito. 
...
starwars_nueva <- 
  starwars %>% 
  drop_na(species) %>% 
  mutate(human_or_not = species == "Human")

starwars_nueva %>% 
  group_by(human_or_not) %>% 
...
.panel[.panel-name[Sol Ej. 9b]

Sin empates (si hay empate, se queda con el primero que hubiera en la tabla)

```{r}
starwars_nueva %>% 
  group_by(human_or_not) %>% 
  slice_min(height, n = 1, with_ties = FALSE) %>%
  ungroup()
```
...
* Se os plantearán **.bg-purple_light[3 ejercicios (con apartados) y deberías encontrar la solución]** correcta y programarla SIGUIENDO LAS INSTRUCCIONES.

--

* Una vez que creáis que la tenéis me llamáis y preguntaré a un **.bg-purple_light[miembro cualquiera del grupo]** que me explique la solución (ergo todos los miembros deberán saber que se ha hecho).

* El **.bg-purple_light[primer grupo]** que me responda correctamente se lleva **.bg-purple_light[2.5 puntos]**, el segundo 1.5, el tercero 1 punto y el cuarto 0.75. Los demás no puntuan.

--

* Ojo: **.bg-purple_light[llamadme cuanto estéis seguros]**. Si me llamáis y la respuesta es incorrecta, se os restará 0.5 puntos.

--

* El grupo con más puntos en cada ejercicio (compuesto de varios apartados) será **.bg-purple_light[invitado a refresco, café, infusión o bebida X (por persona)]** en la cafetería (si un grupo ha ganado un ejercicio, puede volver a ganar el siguiente y se llevaría 2 cada uno)
...
**.bg-purple_light[Primer paso]**: define nuestra base de datos de la mejor manera posible para su futuro uso.
...
Nos piden construir la **.bg-purple_light[tabla de frecuencias]** y para ello, como sucede la hacerlas a mano, lo más importante es $n$ (la frecuencia absoluta). Recuerda: la frecuencia absoluta es simplemente un **.bg-purple_light[conteo]** de los valores que tenemos.
...
A partir de esa tabla podremos construir el resto de columnas de la tabla de frecuencias **.bg-purple_light[tabla de frecuencias]** y para ello, como sucede la hacerlas a mano, lo más importante es $n$ (la frecuencia absoluta).
...
Una de las ventajas de usar R: no necesitaremos la tabla de frecuencias, podemos calcular todo con los datos reales (sin resumir). 

¿Y la media, varianza, sd, CV, mediana y los cuartiles $q_1,q_2,q_3$ (todos devueltos en la misma tabla) SIN usar ningún `$` y SIN usar la tabla de frecuencias (usando los datos originales)? Sin usar dolares ni corchetes
...
datos %>% 
  drop_na(x) %>% 
  summarise(media = mean(x), cuasi_var = var(x), var = mean(x^2) - mean(x),
            sd = sqrt(var), CV = sd / abs(media), mediana = median(x),
            p25 = quantile(x, probs = 0.25), p75 = quantile(x, probs = 0.75))
```
...
En una estación meteorológica se han recogido muestras de valores de concentración de polen en el aire a lo largo de los días,  obteniendo los siguientes datos.

> 174, 185, 166, 176, 145, 167, 191, 174, 159, 155, 181, 160, 170, 198, 180, 150, 161, 183, 172, 163, 147, 176, 141, 170, 171, 158, 184, 173, 169, 162, 172, 181, 187, 177, 164, 171, 193, 183, 173, 179, 167, 178, 180, 168, 173

**.bg-purple_light[Primer paso]**: define nuestra base de datos, **.bg-purple_light[define nuestro tibble]**
...
¿Qué porcentaje de los días se superó los 180 de concentración de polen?
...
Crea una nueva variable llamada `I_i` que nos recategorice la variable agrupándola en intervalos de amplitud 10 (abierto por la izquierda, cerrado por la derecha, empezando desde `"(140, 150]"` el primero y `"(190, 200]"` el último). Necesitamos una tabla de dos columnas: `x` e `I_i` (su categoría)
...
datos_recat <- 
  datos %>% 
  mutate(I_i = case_when(x <= 150 ~ "(140, 150]",
                         x <= 160 ~ "(150, 160]",
                         x <= 170 ~ "(160, 170]",
                         x <= 180 ~ "(170, 180]",
                         x <= 190 ~ "(180, 190]",
...
Inicia una tabla de frecuencias agrupada con tres columnas: I_i (categoría, intervalo) y x_i (marca de clase) y n (frecuencia absoluta). Saca la tabla además con las columnas en ese orden.
...
tabla_frec <- 
  datos_recat %>% 
  count(I_i) %>% 
  mutate(x_i = case_when(I_i == "(140, 150]" ~ 145,
                         I_i == "(150, 160]" ~ 155,
                         I_i == "(160, 170]" ~ 165,
                         I_i == "(170, 180]" ~ 175,
                         I_i == "(180, 190]" ~ 185,
                         TRUE ~ 195)) %>% 
  relocate(x_i, .after = I_i)
tabla_frec
```
...
Esto era solo para practicar: por suerte no necesitaremos perder información agrupando. Usando los datos originales (del primer apartado), calcula la media y mediana, los cuartiles, y las 3 medidas de dispersión en una sola tabla resumen.
...
datos %>% 
  summarise(media = mean(x), mediana = median(x),
            var = mean(x^2) - mean(x)^2, sd = sqrt(var),
            CV = sd / abs(mean(x)), q1 = quantile(x, probs = 0.25),
...
> En la tabla inferior se han recopilado (del año 2013 al 2022) la temperatura media en el mes de abril en Madrid (variable X, en ºC) y el número de días (variable Y) en el que el nivel de ozono superó las 0.20 ppm (partes por millón).
...
¿Cuál fue la media de días en los que se superó el umbral de ozono de 0.20 ppm? (media de la variable Y)
...
Calcula la media de ambas variables, usando solo una vez la orden `mean()` y sin nombrar a las variables.

--

```{r}
datos %>% 
  drop_na(x, y) %>% # por si acaso
...
¿Cuál fue la temperatura media en los años en los que hubo más de 60 días superando el umbral de ozono? (media de X condicionada a Y > 60)
...
¿Cuál fue la media de días en los que se superó el umbral de ozono en los años que la temperatura media en marzo superó los 17.4ºC? (media de Y condicionada a X > 17.4)
...
Calcula a la vez la temperatura media en los años en los que hubo más de 60 días superando el umbral de ozono, y en los que hubo 60 o menos (todo sacado en la misma tabla)
...
¿Existe alguna relación de dependencia entre las variables? ¿De qué tipo? ¿Cómo de fuerte o débil es dicha relación? ¿En qué dirección es dicha relación? Analiza la dependencia y causalidad entre las variables
...
Dependencia lineal muy débil (en caso de existir, positiva), pero no sabemos si puede existir otro tipo de dependencia no lineal. No podemos concluir nada de la causalidad sin combinar la estadística con el conocimiento experto
...
# BLOQUE VIII: análisis de regresión con R

&nbsp;

&nbsp;

### [Análisis numérico y visual](#cor-cov)

### [Ajuste de regresión](#reg)
...
# Análisis .orange[NUMÉRICO]

Vamos a tomar los datos del ejercicio 3 de la hoja 4
...
Tras ello lo **.bg-purple_light[primero es obtener un resumen numérico]** de nuestros datos. Lo podemos sacar todo con `summarise()`
...
* La variable **.bg-purple_light[X es más dispersa]** que la y.

* La correlación es positiva y relativamente elevada: en caso de existir una **.bg-purple_light[relación lineal, será fuerte y positiva]**.

---

# Análisis .orange[VISUAL]

Con el paquete `{corrplot}` podemos tener una **.bg-purple_light[representación visual]**. Por ejemplo, vamos a calcular la **.bg-purple_light[matriz de correlaciones]** (siempre simétrica) de las numéricas de starwars (correlación de todas con todas)
...
Con el paquete `{corrplot}` podemos tener una **.bg-purple_light[representación visual]**.
...
* El **.bg-purple_light[color me indica el signo]** (rojo negativo, azul positivo)

* La **.bg-purple_light[intensidad me indica la magnitud]**
...
Dicho paquete cuenta con distintas opciones de visualización
...
# Fase 1: .orange[AJUSTE]

Volvemos a nuestros datos del ejercicio 3 de la hoja 4. Ya sabemos que la recta de regresión podría funcionar más o menos bien ya que la correlación no es excesivamente baja. **.bg-purple_light[¿Cómo realizar el ajuste?]** Con `lm(data = ..., formula = ...)`
...
* **.bg-purple_light[Coeficientes]**: los tenemos en `reg_lineal %>% coef()` o `reg_lineal$coefficients`
...
* **.bg-purple_light[Valores ajustados]**: los tenemos en `reg_lineal %>% fitted.values()` o `reg_lineal$fitted.values`
...
* **.bg-purple_light[Errores (residuos)]**: los tenemos en `reg_lineal %>% residuals()` o `reg_lineal$residuals`
...
Podemos **.bg-purple_light[añadir esos valores a mis datos originales]** para tenerlo todo en una tabla
...
* Si las horas de estudio son 0, la estimación de mi modelo dice que la nota media será 4.49903

* Por cada hora extra que estudie, mi nota sube 0.122695

---

name: anova

# Fase 3-4: .orange[ANOVA Y EVALUACIÓN]

Con dicha tabla podemos realizar un **.bg-purple_light[análisis de la varianza (ANOVA)]**
...
* El `R2` o **.bg-purple_light[bondad de ajuste]** nos da la proporción de información explicada (fíjate que es definida como la info explicada entre la info total), y que en el caso de la reg. lineal, **.bg-purple_light[coincide con la correlación al cuadrado]** 

* La **.bg-purple_light[media de los residuos]** es siempre 0.

---

# Fase 3-4: .orange[ANOVA Y EVALUACIÓN]

* `%>% summary()`: obtenemos un **.bg-purple_light[resumen automático]**
...
* `%>% anova()`: obtenemos un **.bg-purple_light[ANOVA automático]**
...
# Fase 5: .orange[PREDICCIÓN]

Por último con `predict()` podemos predecir usando cualquier modelo en `R`, pasándole el modelo y los nuevos datos (se tienen que llamar igualq que la variable predictora, en este caso `x`)

```{r}
reg_lineal %>% predict(tibble(x = c(25, 17, 38, 43, 100)))
```

La predicción de los 4 primeros es relativamente fiable (nuestro modelo explicaba más del 50% de la información contenida en Y), pero la última no (de hecho fíjate que la nota está por encima de 10), ya que es un **.bg-purple_light[valor fuera del rango de X]** con el que nuestro modelo ha aprendido: independientemente de la bondad de ajuste, fuera de rango, no sabemos lo que sucede.
...
# BLOQUE IX: visualizando datos

&nbsp;

### [Introducción al dataviz](#intro-historica)
### [Introducción a ggplot](#intro-ggplot2)
...
```{r echo = FALSE,  out.width = "60%", fig.align = "center", fig.cap = "Ejemplo de metáfora visual mal ejecutada"}
...
```{r echo = FALSE,  out.width = "55%", fig.align = "center", fig.cap = "Ejemplo de una buena idea mal ejecutada (de un vicerrector de tecnología...)"}
...
```{r echo = FALSE, out.width = "25%", fig.align = "center", fig.cap = "Ejemplo de buena idea mal ejecutada"}
...
# .orange[GRÁFICOS IMPORTANTES]

Existen muchas formas de **.bg-purple_light[visualizar datos y/o comunicar información]** pero nos vamos a centrar en 5 gráficos importantes:


* **.bg-purple_light[Diagrama de dispersión]**: variables continua vs continua

* **.bg-purple_light[Diagrama de barras]** (o gráfico de columnas): frecuencia de variable discreta (o cuali) o discreta (cuali) vs continua (resumida)

* **.bg-purple_light[Histogramas]**: el equivalente al diagrama de barras para continuas

* **.bg-purple_light[Densidades]**: la versión continua del histograma

* **.bg-purple_light[Gráfico de cajas y bigotes]** (y alternativas)
...
```{r echo = FALSE,  out.width = "55%", fig.align = "center"}
ggplot(gapminder %>% filter(year > 1980),
...
library(showtext)
showtext_auto() # Usar fuentes a futuro
...
# Configurar tema
theme_update(
  plot.title = element_text(color = "#C34539", face = "bold", size = 25),
  plot.subtitle = element_text(color = "#3E6FCB", face = "bold", size = 15),
  plot.caption = element_text(color = "#474A50", face = "bold", size = 9),
  axis.text = element_text(size = 7))

ggplot(gapminder %>% filter(year > 1980),
       aes(x = gdpPercap, y = lifeExp, size = pop)) +
  geom_point(aes(fill = continent), alpha = 0.5,
             color = "white", shape = 21) +
  stat_smooth(method = "lm", se = FALSE,
              color = "#201E1F", size = 0.5,
              linetype = 3) +
  scale_size(range = c(3, 12)) +
  scale_x_log10(
    labels = label_number(suffix = " $", accuracy = 3,
                          decimal.mark = ".")) +
  scale_y_continuous(
    breaks = seq(30, 90, by = 5),
    labels = label_number(suffix = " años")) +
  scale_fill_manual(values = c("#3ac5a3", "#ff8656",
                                "#89a0cf", "#9E2C8B",
                                "#96dc38"),
                     labels = c("África", "Américas", "Asia",
                                "Europa", "Oceanía")) +
  guides(size = "none") +
  ...
  labs(x = "Renta per cápita",
       ...
       color = NULL,
       title = "Gapminder",
       subtitle = "Renta per cápita (log10) vs esperanza de vida (años)",
...
Por último, para simplificar los gráficos, vamos a **.bg-purple_light[filtrar solo algunos términos]** (tras convertirlos a factor en función de su media de probabilidad con `fct_reorder()`)
